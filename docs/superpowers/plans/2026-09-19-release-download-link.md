...
**Architecture:** Use GitHub’s stable `releases/latest` URL for the public repository `mfahryf/XIX-Vectorizer-release`. Keep `VITE_DOWNLOAD_URL` as an optional override, but provide the stable release page as the source default once the public repository exists.

**Tech Stack:** React/Vite, Vitest, existing Vectorizer public page, GitHub Releases.

## Global Constraints

- The button must never point at a version-specific asset name.
...
- [ ] **Step 2: Replace the disabled-link expectation.**
  ...
  Update the public page test to assert that “Download Windows installer” is an external link whose `href` is the stable `releases/latest` URL. Keep a separate test proving `VITE_DOWNLOAD_URL` can override it in the build environment.
...
- [ ] **Step 2: Deploy the web page.**
  ...
  Use the source default, or set `VITE_DOWNLOAD_URL` to the same stable `releases/latest` URL in Coolify. Rebuild `frontend-vectorizer`.

- [ ] **Step 3: Verify the live buttons.**

  Check the hero download button and the installer card at `https://xixlabs.net/vectorizer/`; both must open the latest public release page. Confirm login, trial modal, and preview/carousel behavior remain unchanged.
